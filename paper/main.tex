...
\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{figures/benchmark_comparison.png}
\caption{Headline-level comparison on the 50-clip controlled benchmark; CER and WER decrease,
while exact match and threshold success increase.}
\label{fig:benchmark-comparison}
\end{figure*}

Preprocessing selection alone does not improve center-frame OCR: CER is unchanged (12.34\%
versus 12.34\%), WER is slightly worse (26.33\% versus 26.83\%), and exact match and threshold
success remain 37.5\% and 50.0\%.  Thus the temporal gain cannot be attributed to preprocessing.
...
McNemar result is $p=1.91\times10^{-6}$.  Figure~\ref{fig:benchmark-comparison} makes the four
headline-level changes visible on a common percentage scale.  Center-frame precision, recall, and
F1 are 100.0\%, 50.0\%, and 66.67\%; temporal consensus reaches 100.0\% on all three.
